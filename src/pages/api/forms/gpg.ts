...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const prepracticaValue = normalizeYesNo(req.body?.prepractica);
    ...
    const result = await dbQuery<{ id_grupo: number }>(
      `INSERT INTO gpg (id_grupo, correo, charla, prepractica)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (id_grupo, correo) DO NOTHING
       RETURNING id_grupo`,
      [group_id, email.toLowerCase().trim(), talk, prepracticaValue]
    );

    // Si no insertó, es duplicado
    ...
    return res.status(200).json({
      ...
      console.error("Error en /api/forms/gpg:", err);
