...
const GLOBAL_PASSWORD = process.env.NEXT_PUBLIC_UN_TSS; // Definir la contraseña global aquí // Definir la contraseña global aquí
...
export default function UnPage() {
  ...
  return (
    <div className="p-2 h-screen overflow-auto bg-black text-green-300 font-mono">
      <h1 className="text-lg font-bold mb-2 text-yellow-300 border-b border-yellow-500">UN - Charlas</h1>
      {data?.map((charla: { charla_info: string; participants: any[] }) => (
        <div key={charla.charla_info} className="mb-4 border border-yellow-500 rounded-lg p-2 shadow-lg text-sm bg-gray-900">
          ...
          <table className="w-full border-collapse border border-green-500 text-xs text-green-200 mt-2">
            ...
            <tbody>
              {charla.participants.map((participant: { correo: string; id_grupo: number; nombre?: string; pregrado?: string; semestre?: string; departamentos?: string; asis_assessment?: string }) => (
                <tr key={`${participant.correo}-${participant.id_grupo}`} className="hover:bg-gray-800">
                  ...
                  <td className="border border-green-500 px-2 py-1">{participant.asis_assessment || "N/A"}</td>
                </tr>
              ))}
            </tbody>
          </table>
