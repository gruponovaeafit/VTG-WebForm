...
export default function NovaPage() {
  ...
  const groupedByCharla = data.reduce((acc: Record<string, any[]>, person: any) => {
    const charla = person.charla || "Sin charla";
    if (!acc[charla]) acc[charla] = [];
    acc[charla].push(person);
    return acc;
  }, {});
  ...
  const exportAllNova = allParticipants.map((p: any) => ({
  correo: p.correo,
  nombre: p.nombre,
  ...
  return (
    <div className="p-2 h-screen overflow-auto bg-black text-green-300 font-mono">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-lg font-bold text-yellow-300 border-b border-yellow-500">Nova Dashboard</h1>
        <button
      ...
      {groupByDate ? (
        allParticipants.length > 0 ? (
          <div className="mb-4 border border-yellow-500 rounded-lg p-2 shadow-lg text-sm bg-gray-900">
            ...
            <ExportCSV
  data={exportAllNova}
  filename="nova-todos-los-inscritos.csv"
  label="Descargar CSV"
  columns={[
            ...
            <table className="w-full border-collapse border border-green-500 text-xs text-green-200 mt-2">
              <thead>
                <tr className="bg-gray-700 text-yellow-300">
                  ...
                  <th className="border border-green-500 px-2 py-1">Departamento</th>
              ...
              <tbody>
                {allParticipants.map((p: any, i: number) => (
                  <tr key={`${p.correo}-${i}`} className="hover:bg-gray-800">
                    ...
                    <td className="border border-green-500 px-2 py-1">{p.departamento || "N/A"}</td>
          ...
          return (
            <div key={charla} className="mb-4 border border-yellow-500 rounded-lg p-2 shadow-lg text-sm bg-gray-900">
              ...
              <table className="w-full border-collapse border border-green-500 text-xs text-green-200">
                ...
                <tbody>
                  {personasArray.map((person: any, idx: number) => (
                    <tr key={`${person.correo}-${idx}`} className="hover:bg-gray-800">
                      <td className="border border-green-500 px-2 py-1">{person.correo || "N/A"}</td>
                      ...
                      <td className="border border-green-500 px-2 py-1">{person.departamento || "N/A"}</td>
                      <td className="border border-green-500 px-2 py-1">{person.nombre_miembro || "N/A"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
