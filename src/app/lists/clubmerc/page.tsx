...
const GLOBAL_PASSWORD = process.env.NEXT_PUBLIC_CLUBMERC_TSS; // Definir la contraseña global aquí // Definir la contraseña global aquí
...
export default function ClubmercPage() {
  ...
  return (
    <div className="p-2 h-screen overflow-auto bg-black text-green-300 font-mono">
      <h1 className="text-lg font-bold mb-2 text-yellow-300 border-b border-yellow-500">Clubmerc - Comités</h1>
      {data?.map((comite: { comite: string; participants: any[] }) => (
        <div key={comite.comite} className="mb-4 border border-yellow-500 rounded-lg p-2 shadow-lg text-sm bg-gray-900">
          ...
          <table className="w-full border-collapse border border-green-500 text-xs text-green-200 mt-2">
            ...
            <tbody>
              {comite.participants.map((participant: { correo: string; id_grupo: number; nombre?: string; pregrado?: string; semestre?: string; asistencia_charla?: number }) => (
