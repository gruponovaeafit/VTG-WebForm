...
export default function ClubmercPage() {
  ...
  const exportAllaiesec = allParticipants.map((p: any) => ({
  correo: p.correo,
  nombre: p.nombre,
  ...
  return (
    <div className="p-2 h-screen overflow-auto bg-black text-green-300 font-mono">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-lg font-bold text-yellow-300 border-b border-yellow-500">Clubmerc Dashboard</h1>
        <button
          onClick={() => setGroupByDate(!groupByDate)}
          className="px-4 py-1 bg-green-700 hover:bg-green-600 text-white text-xs rounded"
        >
          {groupByDate ? "Agrupar por comité" : "Listar todos los participantes"}
        </button>
      </div>
      ...
      {groupByDate ? (
        allParticipants.length > 0 ? (
          <div className="mb-4 border border-yellow-500 rounded-lg p-2 shadow-lg text-sm bg-gray-900">
            ...
            <ExportCSV
                          ...
                          filename="Clubmerc-todos-los-inscritos.csv"
                          label="Descargar CSV"
                          columns={[
        ...
        data.map((comite: { comite: string; participants: any[] }) => {
          const participantesArray = comite.participants || [];
          return (
            <div key={comite.comite} className="mb-4 border border-yellow-500 rounded-lg p-2 shadow-lg text-sm bg-gray-900">
              <h2 className="text-md font-semibold mb-1 text-cyan-400">{comite.comite || "Sin comité"}</h2>
              <div className="flex items-center gap-2 text-lg text-green-400 mb-4 pb-3 border-b border-gray-700">
                <Users className="h-5 w-5 text-green-500" />
